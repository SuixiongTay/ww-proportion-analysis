...
representative_ions = {
    ...
    'Gas Scrubber': ['Na[+1]', 'Ca[+2]', 'SO4[-2]'], # Na, Ca, SO4 - should be waste gas treatment? # needs positive ion - Na or Ca
    ...
    'FGD': ['Mg[+2]', 'Ca[+2]', 'Cl[-1]'], # Ca, Mg, Cl # needs positive ion - Ca and Mg
    ...
    'Semiconductor': ['SO4[-2]', 'Ca[+2]', 'F[-1]'], # Ca, F, SO4 # needs positive ion - Ca
    'Tanning': ['Na[+1]', 'Cl[-1]'], # Na, Cl # needs negative ion - Cl
    'Plating': ['Na[+1]', 'Al[+3]', 'SO4[-2]'], # Na, Al, SO4 # needs negative ion - SO4
    'Battery Manufacturing': ['Na[+1]', 'K[+1]', 'SO4[-2]'], # Na, K, SO4 # needs positive ion - Na or K
    'Battery Recycling': ['Na[+1]', 'F[-1]', 'SO4[-2]'] # Na, F, SO4 # needs positive ion - Na
}
...
def citation_rows(df: pd.DataFrame):
    ...
    for row_idx in range(len(citation_col)):
        ...
        if pd.notna(citation_name) and citation_name != current_citation and current_citation is not None:
            # save citation data
            columns_with_data = []
...
class ww_proportion_calculator:
    ...
    def _std_element_to_std_ion_dict(self, final_eq_prop: dict, final_pH: float):
        ...
        for element, conc in final_eq_prop.items():
            ...
            if "(unk)" in str(element) or "unk" in str(element): # S(unk) is present when using llnl.dat
                continue
    ...
    def build_final_eq_from_initial_median_concentration(self):
        ...
        initial_sol = Solution(solutes=ion_dict_for_solution, pH=initial_median_pH, engine=custom_eos)
        ...
        for target_ion in self.REPRESENTATIVE_IONS:
            ...
            charge_balance_logic = (
                (initial_chg_bal < 0 and is_positive_ion) or 
            ...
            self.final_element_proportions_from_initial_median[self.industry][std_target_ion] = {
                "solutes": final_sol.as_dict()["solutes"], #not working for some reason
            ...
            for element in final_sol.get_components_by_element().keys():
                ...
                self.final_element_proportions_from_initial_median[self.industry][std_target_ion]["proportion_percent"][element] = {
                    "concentration_mg_L": final_val,
    ...
    def build_proportion_from_final_eq_initial_median(self):
        ...
        """
        final_props_from_initial_median = self.final_element_proportions_from_initial_median
        ...
        for industry, std_target_ions in final_props_from_initial_median.items():
            ...
            if len(std_target_ions) == 1:
                ...
                for std_target_ion, target_data in std_target_ions.items():
                    ...
                    solutes = {k: v for k, v in (target_data.get("solutes") or {}).items() if "unk" not in str(k)}
    ...
    def final_equilibration_for_initial_median(self):
        ...
        median_conc_dict = self.report_equilibrated_proportions[self.industry].get("init_dict", None)
        ...
        for industry, data in self.report_equilibrated_proportions.items():
            ...
            if rep_ion_count is not None and rep_ion_count == 1:
                try:
                    final_eq_sol = Solution(solutes=median_conc_dict, pH=median_pH_dict, engine=custom_eos)
                ...
                try:
                    final_eq_sol = Solution(solutes=median_conc_dict, pH=median_pH_dict, balance_charge="auto", engine=custom_eos)
                    final_eq_sol.equilibrate()
                    equilibrated = True
                except Exception as e:
                    return f"failed to equilibrate: {e}"
...
def execution(df: pd.DataFrame, REPRESENTATIVE_IONS: list, industry: str):
    ...
    prop_from_initial_medians = ww_prop.build_final_eq_from_initial_median_concentration()

    eq_prop_from_initial_medians = ww_prop.build_proportion_from_final_eq_initial_median()
    ...
    return ion_dicts, initial_median_conc, prop_from_initial_medians, eq_prop_from_initial_medians, final_eq_from_initial_medians
