...
class ww_proportion_calculator:
    ...
    def final_equilibration_step(self):
        ...
        initial_sol = Solution(solutes=median_conc_dict, pH=self.median_final_pH, engine="phreeqc2026")
        initial_chg_bal = initial_sol.charge_balance

        equilibrated = False
        # include charge balance logic here
        for target_ion in self.REPRESENTATIVE_IONS:
            # standardized target ion formula
            std_target_ion = standardize_formula(target_ion)
            ...
            # equilibrated = False
            if target_ion in median_conc_dict and charge_balance_logic:
                try:
                    final_eq_sol = Solution(solutes=median_conc_dict, pH=self.median_final_pH, balance_charge=target_ion, engine="phreeqc2026")
                    final_eq_sol.equilibrate()
                    equilibrated = True
                except:
                    continue

        # final_eq_sol = Solution(solutes=median_conc_dict, pH=self.median_final_pH, balance_charge="auto", engine="phreeqc2026") # should not be auto
        # final_eq_sol.equilibrate()
